...
func gitNativeSetup(ctx context.Context, repoURL, dir, rev string) error {
	...
	cmd := run.Shellf(ctx, "git clone %s %s", repoURL, dir)
	cmd.Stdout, cmd.Stderr = os.Stdout, os.Stderr // Cloning is slow, let the user see progress.
	err = cmd.Run()
	if err != nil {
		return fmt.Errorf("native git cloning %s into %s: %w", repoURL, dir, err)
	}

	cmd = run.Shellf(ctx, "git checkout %s", rev)
	cmd.Dir = dir
